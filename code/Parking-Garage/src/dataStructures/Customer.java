...
public class Customer extends Database{
	...
	/*	Purpose:	Have the customer check out
	 * 	Input:		current time
	 * 	Output:		True if successful check out, false if the check out fails.
	 */
	// TODO - Is this necassary
	public boolean checkout (Calendar currentTime) throws ClassNotFoundException, SQLException{
		int tempCurrentReservation = currentReservation;
		int tempParkingSpot;
		...
		currentReservation = -1;
		pastReservations.remove(0); //delete oldest
		pastReservations.add(tempCurrentReservation); //add this one just passed
		Customer.Customer_to_Database(this);

		// update the actual check out time for the reservation ticket
		Reservation tempRes = Reservation.ID_to_Reservation(tempCurrentReservation);
		tempParkingSpot = tempRes.getParkingSpot();
		tempRes.setActualTimeOut(currentTime); 
		Reservation.Reservation_to_Database(tempRes);

		// update the Lot Monitor that the car is not in its spot
		LotMonitor.reservationCheckOut(tempParkingSpot);
		//Checkout earlier than expected?
		if (tempRes.reservationInformation().get(1).compareTo(currentTime)>0){ //earlier
			LotMonitor.unReserveParkingSpace(tempParkingSpot, tempRes.reservationInformation().get(1), currentTime);
		}
